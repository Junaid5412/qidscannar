...
function enrich_qid_record($record) {
    ...
    // Due Date Status
    $due_alert_days = (int)get_setting('due_alert_days', 7);
    if (!empty($record['payment_due_date']) && $record['remaining_balance'] > 0) {
        $days_to_due = calculate_days_left($record['payment_due_date']);
        $record['days_to_due'] = $days_to_due;
        if ($days_to_due < 0) {
            $record['due_status'] = 'Overdue';
            $record['due_badge'] = '<span class="badge bg-danger text-white"><i class="fa-solid fa-bell me-1"></i>Overdue (' . abs($days_to_due) . 'd)</span>';
        } elseif ($days_to_due <= $due_alert_days) {
            $record['due_status'] = 'Due Soon';
            $record['due_badge'] = '<span class="badge bg-warning text-dark"><i class="fa-solid fa-calendar-clock me-1"></i>Due in ' . $days_to_due . 'd</span>';
        } else {
            $record['due_status'] = 'Upcoming';
            $record['due_badge'] = '<span class="badge bg-light text-secondary border">' . format_date($record['payment_due_date']) . '</span>';
        }
    } else {
        $record['days_to_due'] = null;
        $record['due_status'] = ($record['remaining_balance'] <= 0) ? 'Settled' : 'None';
        $record['due_badge'] = ($record['remaining_balance'] <= 0) 
            ? '<span class="badge bg-light text-muted">Settled</span>' 
            : '<span class="badge bg-light text-muted">No Due Date</span>';
    }
